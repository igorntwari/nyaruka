...
export default function BusinessHistoryPage() {
  ...
  return (
    <main className="dash-shell px-6 pt-10 pb-28 lg:px-10">
      <p className="font-display font-bold text-2xl text-ink mb-6">Delivery history</p>

      {orders.length === 0 ? (
        <div className="text-center pt-16">
          <p className="font-body text-ink-soft text-sm">No delivery requests yet.</p>
          <Link href="/order/new" className="text-route font-semibold font-body text-sm">Submit your first request</Link>
        </div>
      ) : (
        <>
          <div className="flex flex-col sm:flex-row gap-3 mb-6">
            ...
            <div className="sm:w-56 shrink-0">
              <Select label="Status" hideLabel options={STATUS_OPTIONS} value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as OrderStatus | 'ALL')} />
            </div>
          </div>
          ...
          {filtered.length === 0 && (
            <div className="text-center pt-16">
              ...
              <p className="font-body text-sm text-ink-soft">No requests match your search.</p>
            </div>
          )}
